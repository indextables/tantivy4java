...
/// Extract the full config from the Java Map into a Rust HashMap.
///
/// Copies the entire map rather than whitelisting specific keys. A whitelist
/// silently drops any key it doesn't enumerate — that already caused
/// `session.timezone.offset.seconds` to be dropped, disabling timestamp-timezone
/// data skipping. Copying everything is simpler and future-proof: new config keys
/// (cache TTLs/capacities, `max_concurrent_reads`, `checkpoint_interval`,
/// `session.timezone.offset.seconds`, etc.) are passed through automatically.
fn extract_extended_config(env: &mut JNIEnv, config_map: &JObject) -> std::collections::HashMap<String, String> {
    crate::common::extract_hashmap(env, config_map).unwrap_or_default()
}
