...
//
// For V1 parquet files (no native offset index in their footer), this module
...
//
// Only flat columns (max_rep_level == 0) are supported — for these columns,
// num_values == num_rows so first_row_index can be computed exactly from the
// page headers alone. Nested columns (List/Map/Struct with max_rep_level > 0)
// are skipped at index time; CachedParquetReader synthesizes a full-chunk
// download for them instead.
//
...
/// Compute page locations for a single flat column chunk by scanning
/// Thrift-encoded page headers from the raw column chunk bytes.
///
/// This is used for V1 parquet files that don't have an offset index
/// in their footer. The computed locations enable page-level byte
/// range reads at query time.
///
/// Only call this for flat columns (max_rep_level == 0) where
/// num_values == num_rows. Nested columns should be handled by
/// CachedParquetReader with a full-chunk download.
///
/// # Arguments
...
pub fn compute_page_locations_from_column_chunk(
    ...
    while pos < chunk_len {
        ...
        match header.page_type {
            PAGE_TYPE_DATA_PAGE | PAGE_TYPE_DATA_PAGE_V2 => {
                // Flat column: num_values == num_rows
                pages.push(PageInfo {
                    file_offset: page_file_offset as i64,
                    total_page_size,
                    num_rows: header.num_values as i64,
                });
            }
