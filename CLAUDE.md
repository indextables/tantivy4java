...
#### **🌟 Cross-Reference (XRef) Splits for Query Routing (December 2025)**

**✅ Complete XRef Split Implementation:**
- **✅ XRefSplit.build()** - Create lightweight index consolidating term dictionaries from N source splits
- **✅ Standard SplitSearcher Compatibility** - XRef splits are standard Quickwit splits, searchable with regular `SplitSearcher`
- **✅ Footer-Only Downloads** - Opens source splits using only footer offsets (not entire files)
- **✅ Multi-Cloud Support** - Full support for local files, S3, and Azure Blob Storage
- **✅ Query Routing** - Quickly identify which splits contain matching documents before full search
...
**XRef Split Build Example:**
```java
// Create XRef from source splits (footer offsets from metadata)
...
**Search XRef with Standard SplitSearcher (Recommended):**
```java
// XRef splits are standard Quickwit splits - use regular SplitSearcher
try (SplitCacheManager cacheManager = SplitCacheManager.getInstance(cacheConfig);
    ...
    for (var hit : matchingSplits.getHits()) {
        try (Document doc = searcher.doc(hit.getDocAddress())) {
            ...
            System.out.println("Found term in split: " + splitId);
...
**XRef Internal Document Fields:**
- `_xref_uri` - Source split URI for retrieval
- `_xref_split_id` - Split identifier
- `_xref_split_metadata` - Full split metadata as JSON (footer offsets, num_docs, size_bytes)

**Design Principle:** If XRef references 1,000 source splits, it contains exactly 1,000 documents - one per split. Posting lists point to split-level documents, NOT original documents.
